...
def upsampleL(fieldmap, activation_data, reduction=1, shape=None,
    ...
    shape: the shape of output/target
    reduction: the difference between segmentation size and input image size, =input_w / segmentaion_w
    '''
...
def layer_fieldmap(layer):
    ...
    if layer.type == 'Convolution' or layer.type == 'Pooling':
        ...
        # field map: (offset, size, step)
        # offset: the padding of each layer, not accumulated one
